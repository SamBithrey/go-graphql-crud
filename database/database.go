...
var connectionString string = "mongodb+srv://sambithrey:sambithrey@cluster0.cpmgvox.mongodb.net/?retryWrites=true&w=majority"
...
func (db *DB) GetJob(id string) *model.JobListing {
	jobCollection := db.client.Database("job-board").Collection("jobs")
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
...
func (db *DB) UpdateJobListing(jobId string, jobInfo model.UpdateJobListingInput) *model.JobListing {
	...
	updatedJob := jobCollection.FindOneAndUpdate(ctx, filter, update, options.FindOneAndUpdate().SetReturnDocument(1))

	var jobListing model.JobListing

	if err := updatedJob.Decode(&jobListing); err != nil {
		log.Fatal(err)
	}
	return &jobListing
}
